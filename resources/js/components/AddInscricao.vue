<template>
    <div>
        <h4 class="text-center">Adicionar Inscrição</h4>
        <div class="row">
            <div class="col-md-6">
                <form @submit.prevent="AddInscricao">
                    ...
                    <button type="submit" class="btn btn-primary">Adicionar Inscrição</button>
                </form>
            </div>
        </div>
    </div>
    ...
    methods: {
        AddInscricao() {
            this.$axios.get('/sanctum/csrf-cookie').then(response => {
                this.$axios.post('/api/cursos/add', this.inscricao)
                    .then(response => {
